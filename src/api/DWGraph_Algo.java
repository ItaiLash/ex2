...
public class DWGraph_Algo implements dw_graph_algorithms {
    ...
    private double Dijkstra(node_data src, node_data dest) {
        ...
        tempSrc.setDis(0.0); //distance
        //src.setTag(1);  //visited
        pq.add(src);
        ...
        while (!pq.isEmpty()) {
          node temp = (node)pq.poll();
            for (edge_data e : this.graph.getE(temp.getKey())) {
                ...
                if (n.getTag() == -1) {
                    if (n.getDis() > temp.getDis() + n.getLocation().distance(temp.getLocation())) {
                        n.setDis(Math.min(n.getDis(), temp.getDis() + n.getLocation().distance(temp.getLocation())));
                        n.setPre(temp);
                    }
                    pq.add(n);
                }
